...
for zone_name in `cat $VAR_DIR/input/zones/zones.inp`; do
  ...
  for auth_name in `grep open_wlan $VAR_DIR/input/wlans/$zone_name.inp`; do
    echo "start time:`date`"
    echo "$auth_name $zone_id"
    ./create_non_proxy_auth_service.sh $auth_name $zone_id | tee $VAR_DIR/output/wlans/${zone_name}_${wlan_name}.out
    echo "end time:`date`"
  done
done
echo "end job:`date`"
