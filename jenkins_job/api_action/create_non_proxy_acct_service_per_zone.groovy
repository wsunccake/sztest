...
for name in `cat $VAR_DIR/input/zones/zones.inp`; do
  ...
  cat -n $VAR_DIR/input/non_proxy_acct/$zone_name.inp | xargs -P $NPROC -n 2 sh -c './create_non_proxy_acct_service.sh acct$1.$0 $1 $radius_port $radius_secret $zone_id | tee $VAR_DIR/output/proxy_acct/${zone_name}_$1.$0.out'
