...
for domain_name in `cat $VAR_DIR/input/partner_domains/domains.inp`; do
  ...
  # create zone
  cat $VAR_DIR/input/device_policy/$domain_name.inp | xargs -P $NPROC -i sh -c "./create_domain_device_policy.sh {} $domain_id | tee $VAR_DIR/output/device_policy/${domain_name}_{}.out"
...
'''
        ...
        stage('Check Response') {
            steps {
                script {
                    def cmd1 = ["bash", "-c", "grep 'Response code:' ${VAR_DIR}/output/device_policy/*.out | wc -l"]
