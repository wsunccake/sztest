...
for domain_name in `cat $VAR_DIR/input/partner_domains/domains.inp`; do
  ...
  # create zone
  cat $VAR_DIR/input/proxy_acct/$domain_name.inp | xargs -P $NPROC -i sh -c "./create_acct_service.sh {} {} $radius_port $radius_secret $domain_id | tee $VAR_DIR/output/proxy_acct/${domain_name}_{}.out"
...
'''
        ...
        stage('Check Response') {
            steps {
                script {
                    def cmd1 = ["bash", "-c", "grep 'Response code:' ${VAR_DIR}/output/proxy_acct/*.out | wc -l"]
                    def proc1 = Runtime.getRuntime().exec((String[]) cmd1.toArray())
                    def totalResponse = proc1.text.trim() as Integer

                    def cmd2 = ["bash", "-c", "grep 'Response code: 201' ${VAR_DIR}/output/proxy_acct/*.out | wc -l"]
